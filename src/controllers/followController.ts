...
export const getFollows = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!targetUserId) {
      return res
        .status(400)
        .json({ status: "error", message: "userId tidak valid" });
    }
...
export const followUser = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!followed_user_id) {
      return res
        .status(400)
        .json({ status: "error", message: "followed_user_id is required" });
    }

    if (followerId === followed_user_id) {
      return res
        .status(400)
        .json({ status: "error", message: "Tidak bisa follow diri sendiri." });
    }
    ...
    if (existing) {
      return res
        .status(400)
        .json({ status: "error", message: "Kamu sudah follow user ini." });
    }
...
export const unfollowUser = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!followed_id) {
      return res
        .status(400)
        .json({ status: "error", message: "followed_id is required" });
    }
